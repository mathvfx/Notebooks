...
KNN::CompleteBinaryTree::CompleteBinaryTree(size_t max_number_of_nodes) {
...
size_t KNN::CompleteBinaryTree::height(size_t node_idx) const {
    return std::floor(std::log2(node_idx+1));
}

const KNN::CBT_POD& KNN::CompleteBinaryTree::root() const {
    return cbt_pod_list[0];
}

const KNN::CBT_POD& KNN::CompleteBinaryTree::parent(size_t node_idx) const {
    size_t idx = node_idx > 0 ? (node_idx - 1)/2 : 0;
        // Using integer division truncation to act as flooring there.
    return cbt_pod_list[idx];
}

const KNN::CBT_POD& KNN::CompleteBinaryTree::left_child(size_t node_idx) const {
...
const KNN::CBT_POD& KNN::CompleteBinaryTree::right_child(size_t node_idx) const  {
...
bool KNN::CompleteBinaryTree::has_node(size_t node_idx) const {
    return static_cast<bool>(cbt_pod_list[node_idx].ptcoord);
}

bool KNN::CompleteBinaryTree::has_left_child(size_t node_idx) const {
    const size_t idx = 2*node_idx + 1;
    return idx < reserved_capacity && cbt_pod_list[idx].ptcoord; // FIXME: change cbt_pod_list after add_*_child stuff implemented
...
bool KNN::CompleteBinaryTree::has_right_child(size_t node_idx) const {
    const size_t idx = 2*node_idx + 2;
...
size_t KNN::CompleteBinaryTree::left_child_idx(size_t node_idx) const {
    return 2*node_idx + 1;
}

size_t KNN::CompleteBinaryTree::right_child_idx(size_t node_idx) const {
    return 2*node_idx + 2;
}

size_t KNN::CompleteBinaryTree::add_node(CBT_POD& pod_data, size_t node_idx) {
...
size_t KNN::CompleteBinaryTree::add_node(CBT_POD&& pod_data, size_t node_idx) {
    ...
    ++num_nodes;
    return node_idx; // 0 is the index of root node in this array-based binary tree
}
...
size_t KNN::CompleteBinaryTree::add_root(CBT_POD&& pod_data) {
    return add_node(std::move(pod_data), 0); // 0 is the index of root node in this array-based binary tree
}
...
// TODO: DEPRECATION. To be cleaned up.


void KNN::CompleteBinaryTree::append(CBT_POD& elem) {
    ++num_nodes;
    cbt_pod_list.push_back(elem);
}
